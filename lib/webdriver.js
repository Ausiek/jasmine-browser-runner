...
export function buildWebdriver(browserInfo, webdriverBuilder) {
  const Capability = seleniumWebdriver.Capability;

  webdriverBuilder = webdriverBuilder || new seleniumWebdriver.Builder();
  ...
  if (!useSauce) {
    if (browserName === 'headlessChrome') {
      const caps = seleniumWebdriver.Capabilities.chrome();
